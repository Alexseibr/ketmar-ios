...
open class Router: NSObject, RouterProtocol {
    ...
    public func setRootModule(_ module: Presentable?, transitionOptions: [TransitionOption]) {
        guard let controller = module?.toPresent() else { return }
        let config: TransitionConfig = .init().with(options: transitionOptions)
    ...
    public func contains<T>(_: T.Type) -> Bool {
        rootController?.viewControllers.contains(T.self) ?? false
    }

    public func child<T>(_: T.Type) -> T? {
        rootController?.viewControllers.first(T.self)
    }
