...
export default function ConversationsPage() {
  ...
  const getConversationCountText = () => {
    if (!user) return 'Войдите для просмотра';
    if (conversations.length === 0) return 'Нет активных диалогов';
    const count = conversations.length;
    if (count === 1) return '1 диалог';
    if (count < 5) return `${count} диалога`;
