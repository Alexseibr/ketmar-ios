...
router.get('/my/shop-request', authMiddleware, async (req, res) => {
  try {
    const user = req.currentUser;
    ...
    console.error('[SellerProfile] Get my shop request error:', error);
    res.status(500).json({
      success: false,
      error: 'server_error',
    });
...
router.post('/shop-request', authMiddleware, async (req, res) => {
  try {
    const user = req.currentUser;
    ...
    if (existingProfile && existingProfile.isVerified) {
      return res.status(400).json({
        success: false,
        error: 'already_has_store',
        message: 'У вас уже есть верифицированный магазин',
      });
    }
    ...
    if (existingRequest) {
      return res.status(400).json({
        ...
        message: 'У вас уже есть активная заявка на модерации',
    ...
    console.log(`[SellerProfile] Shop request created by user ${user.telegramId}: ${name}`);
    ...
    console.error('[SellerProfile] Create shop request error:', error);
    res.status(500).json({
      success: false,
      error: 'server_error',
